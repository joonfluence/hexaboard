...
**Phase 1 후 재확인**: 통과. 다만 아래 "확인이 필요한 결정"의 1번은 문서에 근거가 없어 사용자 확인 대상이다.

## 확인이 필요한 결정 (문서에 명시 없음)

1. **생성 시 순서 키 충돌 처리**: 두 생성 요청이 동시에 같은 "마지막 카드 뒤" 키를 계산하면 (상태, 순서 키) 유니크 제약에 걸린다. 문서(D-69)는 "동시 이동"의 재시도(최대 3회 후 `409`)만 정한다. **제안**: 생성에도 같은 재시도 상수를 재사용하고 초과 시 `409 POSITION_CONFLICT`를 돌려준다. 확정되면 결정 로그에 기록한다.
2. **품질 게이트 스크립트 이름**: TRD 05가 "이름 미정"으로 남겼다. **제안**: Turborepo 태스크 `typecheck`, `lint`, `test`, `build`, `dev`.
