...
| TC-PER-030 | 이웃 순서 키 조회(앞·뒤) | 같은 상태에서 기준 바로 앞·뒤 키. 없으면 `null`. 제외한 티켓·다른 상태는 무시 | 004 | ✅ |
| TC-PER-031 | 컬럼에 (제외 티켓 말고) 카드가 있는지 | 있으면 `true`, 없거나 제외 티켓뿐이면 `false` | 004 | ✅ |
| TC-PER-032 | 이동 저장 | 상태·순서 키만 바뀌고 제목 등은 그대로, `updatedAt` 갱신 | 004 | ✅ |
| TC-PER-033 | 이미 쓰는 (상태, 순서 키)로 이동 | `PositionConflictError`, 티켓은 그대로 | D-49 | ✅ |
| TC-PER-034 | 없는 티켓 이동 | `null` | 004 | ✅ |
| TC-PER-035 | 상태별 조회 | 그 상태 카드만 순서 키 오름차순 | FR-09 | ✅ |
| TC-PER-036 | 순서 다시 쓰기 | 주어진 순서대로 새 키가 저장되고 다른 상태·내용은 불변, 기존 키와 겹치는 새 키여도 성공 | FR-09, D-98 | ✅ |
| TC-PER-037 | 태그와 함께 저장 후 조회 | 이름 오름차순 태그가 돌아옴 | FR-07 | ✅ |
| TC-PER-038 | 두 티켓이 같은 태그 이름 | `tag` 행은 하나(이름 유니크) | data_model | ✅ |
| TC-PER-039 | 수정 저장 | 태그가 교체됨(추가·제거), `[]`는 모두 제거 | D-99 | ✅ |
| TC-PER-040 | 목록·상태별 조회·이동 결과 | 각 티켓의 태그를 함께 돌려줌 | D-99 | ✅ |
| TC-PER-041 | 티켓 삭제 | 그 티켓의 연결만 사라지고 다른 티켓의 태그는 그대로 | D-99 | ✅ |
| TC-PER-042 | 마이그레이션 | `tag`·`ticket_tag` 생성, 태그 이름 유니크·연결 복합 PK | data_model | ✅ |
| TC-PER-043 | 검색어 필터 | 제목·설명 부분 일치, 대소문자 무시, `%`·`_`는 글자 그대로 | FR-08, D-100 | ⏳ |
| TC-PER-044 | 상태·우선순위 필터 | 같은 종류 여러 값은 OR | FR-08 | ⏳ |
| TC-PER-045 | 태그 필터 | 여러 태그 중 하나라도 가진 티켓(OR), 태그 이름은 정규화해 비교 | FR-08, D-64 | ⏳ |
| TC-PER-046 | 여러 종류의 조건 | 모두 만족하는 티켓만(AND), 결과 순서는 필터 없는 목록과 같음 | FR-08 | ⏳ |
