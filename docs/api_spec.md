...
응답에는 내부 PK를 포함하지 않고 공개 식별자 `ticketId`만 둔다. 필드 이름은 camelCase로 두는 것을 초안으로 한다. 응답에 `position`을 노출할지는 미정이다 (목록이 이미 정렬되어 오므로 필수가 아닐 수 있음).
...
- 요청·응답 스키마 상세, 응답의 `position` 노출 여부
- 필터 파라미터의 이름·형식
